...
def generate_cpp_script(characters, output_dir_suffix=""):
    """Generate a C++ script for the given character set"""
...
const std::string characters = "{characters}", output_dir = "output{suffix}/";
...
int main() {{
  ...
  return 0;
    ...
    return template.format(characters=characters, suffix=output_dir_suffix)
...
def main():
    ...
    for chars, dirname in character_sets:
        # Create directory if it doesn't exist
        if not os.path.exists(dirname):
            os.makedirs(dirname)

        # Generate and write the C++ file
        cpp_content = generate_cpp_script(chars)
        cpp_path = os.path.join(dirname, "main.cpp")
